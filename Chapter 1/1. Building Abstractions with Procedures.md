...
如果 Lisp 不是主流語言，為什麼我們要挑選 Lisp 作為探討程式設計的框架呢? 因為Lisp 擁有獨一無二的特性，使它成為學習重要的程式建構與資料結構的完美媒介，並且這些特性相互關連從而支持程式建構。這些特性中最重要的一點，是Lisp能以資料的形式表示與操作程序(process)。這個特性之所以重要，是因為某些很有用的程式設計技巧仰賴於此，以淡化傳統靜態資料與動態程序間的分野。將程序以資料方式呈現的能力，也使Lisp成為一門很棒的語言，可撰寫必須操控其他程式的程式，例如：支持電腦語言的解譯器和編譯器。綜上所述，用Lisp寫程式是非常有趣的。
